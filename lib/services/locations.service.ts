...
export default class LocationService {
  ...
  async getLocationsByType(type: "kebele" | "district" | "zone" | "region") {
    try {
      const { data, error } = await supabase
        .from("locations")
        .select(
  ...
  async getLocationsByCampaign(campaignId: string) {
    try {
      const { data, error } = await supabase
  ...
  async getPromoterLocations(promoterId: string): Promise<Location[]> {
  ...
  async addLocationToCampaign(campaignId: string, locationId: string) {
    const { data, error } = await supabase
      .from("campaign_locations")
      .insert([
        {
